...
Form the above graph it is clear that 1107 leopards  and 933 tigers have 10 or less number of images. Moreover over `r sum(final_plot$count[1:5])` out of `r sum(final_plot$count[1:6])` leopards and   `r sum(final_plot$count[7:11])` out of `r sum(final_plot$count[7:12])` tigers individuals fall in the range of less that 50 images.  
